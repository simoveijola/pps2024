...
const float u_x=-1 , u_y=-0.5 , c_amp=1;  // choose velocity components and amplitude of initial condition
...
{   
	...
	rhs(xrange_1, yrange_2, subdomain_ny, data, d_data);
		...
		if(iter == iterations-1) fprintf(fptr_approx,"%f ",data[ix][iy]);
            }
	    if(iter == iterations-1) fprintf(fptr_approx,"\n");
        }
	if(iter == iterations-1) fprintf(fptr_approx,"\n");
